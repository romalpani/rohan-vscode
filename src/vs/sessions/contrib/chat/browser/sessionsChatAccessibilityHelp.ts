...
export class SessionsChatAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor) {
		...
		content.push(localize('sessionsChat.sessionActions', "For a created session, focus the session title in the title bar and press Enter or Space to open its actions menu. The available actions include starting or switching chats, renaming, pinning, closing, maximizing or restoring, archiving or restoring, and marking the session read or unread."));
		content.push(localize('sessionsChat.conversations', "When a session supports multiple chats, choose New Chat from the session title's actions menu to start one. The Chats submenu appears once the session has more than one committed chat or the active chat has subagents. Side chats appear as first-level chats. A Subagents group lists work delegated by the active chat, and every item announces its state. When there is one first-level chat, only its Subagents are listed. The active chat or subagent is selected when the submenu opens. Select an item to open or focus it."));
		...
		content.push(localize('sessionsChat.renameSession', "To rename the active session, open the session title's actions menu and choose Rename. Edit the title in place, then press Enter to save or Escape to cancel. Moving focus away also cancels the edit. You can also double-click its title in the sessions list, or focus the list item, open its context menu (for example, with Shift+F10), and choose Rename."));
